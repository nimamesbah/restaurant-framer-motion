import axios from "axios";
import React, { useEffect } from "react";
import useGetProduct from "../hooks/useGetproducts";
import HeaderCard from "./product-cards/Cards";

function Header() {
  const { data, isError, isLoading } = useGetProduct();
  useEffect(() => {
    console.log(data);
  }, [data]);
  return (
    <>
      <div id="headerContiner" className="w-full h-[600px] ">
        <div id="shade" className="w-full backdrop-brightness-[35%] h-full">
          <div className="absolute top-[40%] left-[50%] -translate-1/2 flex flex-col gap-4 justify-center items-center text-white ">
            <h1 className="text-4xl">feliciano</h1>
            <p className="text-3xl w-max">best restaurant</p>
          </div>
          <div id="headerContent" className="w-full h-full">
            <div
              id="numEmailPcCont"
              className="w-full  max-sm:hidden bg-topNav py-2 text-topNavText font-mono"
            >
              <div className="w-mainW mx-auto flex justify-between items-center max-w-container">
                <div className="flex justify-between gap-3 ">
                  <svg
                    width="20px"
                    viewBox="0 0 24 24"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      d="M16.5562 12.9062L16.1007 13.359C16.1007 13.359 15.0181 14.4355 12.0631 11.4972C9.10812 8.55901 10.1907 7.48257 10.1907 7.48257L10.4775 7.19738C11.1841 6.49484 11.2507 5.36691 10.6342 4.54348L9.37326 2.85908C8.61028 1.83992 7.13596 1.70529 6.26145 2.57483L4.69185 4.13552C4.25823 4.56668 3.96765 5.12559 4.00289 5.74561C4.09304 7.33182 4.81071 10.7447 8.81536 14.7266C13.0621 18.9492 17.0468 19.117 18.6763 18.9651C19.1917 18.9171 19.6399 18.6546 20.0011 18.2954L21.4217 16.883C22.3806 15.9295 22.1102 14.2949 20.8833 13.628L18.9728 12.5894C18.1672 12.1515 17.1858 12.2801 16.5562 12.9062Z"
                      fill="#ffffff80"
                    />
                  </svg>
                  <h1 className="font-bold font-mono">+0910344566</h1>
                </div>
                <div className="flex justify-between gap-3">
                  <svg
                    width="20px"
                    viewBox="0 0 24 24"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      fill-rule="evenodd"
                      clip-rule="evenodd"
                      d="M21.3489 4.64183C21.4734 4.76317 21.526 4.94053 21.4877 5.11016C20.9359 7.55335 19.7598 13.9696 19.4822 20.0229C19.474 20.2015 19.3712 20.362 19.2124 20.4441C19.0537 20.5262 18.8632 20.5173 18.7127 20.4208C16.3647 18.9142 11.6602 16.0278 11.0091 15.6323C10.8858 15.5573 10.8008 15.4327 10.7761 15.2905C10.7514 15.1482 10.7894 15.0023 10.8803 14.8901C10.9514 14.8024 11.0276 14.706 11.1105 14.6011C11.5439 14.0526 12.1621 13.2704 13.2013 12.2766C13.4009 12.0858 13.7174 12.0929 13.9083 12.2925C14.0991 12.4921 14.092 12.8086 13.8924 12.9994C12.9904 13.862 12.432 14.5445 12.0123 15.0733C13.2789 15.8492 16.4196 17.7821 18.5284 19.1177C18.8395 13.8342 19.7665 8.44365 20.3378 5.69741C16.0184 6.93311 8.54822 10.0196 4.32406 11.8562L7.98141 13.2367C9.62636 12.1619 13.2647 9.91045 16.2836 8.46108C16.5325 8.34157 16.8312 8.44648 16.9507 8.69542C17.0703 8.94436 16.9653 9.24305 16.7164 9.36257C13.6191 10.8496 9.82741 13.2144 8.3188 14.2115C8.18492 14.3 8.01669 14.3189 7.86654 14.2622L2.82343 12.3587C2.63357 12.287 2.50586 12.1077 2.5002 11.9048C2.49453 11.702 2.61203 11.5158 2.7976 11.4337C6.88855 9.62258 16.2667 5.68497 20.8771 4.51536C21.0456 4.4726 21.2243 4.52049 21.3489 4.64183Z"
                      fill="#ffffff80"
                    />
                  </svg>
                  <h1 className="font-semibold">karajStreet@karaj.com</h1>
                </div>
                <div className="font-semibold">
                  Open hours: Monday - Sunday 8:00AM - 9:00PM
                </div>
              </div>
            </div>
            <div
              id="navbarContPc"
              className=" flex-col justify-between hidden sm:flex text-white border border-topNavBorder w-full h-[550px]"
            >
              <div
                id="navbarPc"
                className="w-mainW flex justify-center mx-auto gap-8 py-4 items-center capitalize border-b border-topNavBorder"
              >
                <h2 className="grow text-2xl cursor-pointer ">feliciano</h2>
                <h2 className="text-themeGold cursor-pointer">home</h2>
                <h2 className="cursor-pointer">about</h2>
                <h2 className="cursor-pointer">stories</h2>
                <h2 className="cursor-pointer">contact</h2>
                <h2 className="cursor-pointer">menu</h2>
                <button className="bg-themeGold rounded-sm py-4 px-4 cursor-pointer">
                  Book a table
                </button>
              </div>
              <div
                id="4FoodAd"
                className="flex w-mainW gap-4 mx-auto justify-between items-center border-t border-topNavBorder py-3.5"
              >
                {isLoading ? (
                  <h1>loading.....</h1>
                ) : (
                  data
                    .map((item) => (
                      <HeaderCard name={item.name} image={item.image} />
                    ))
                    .slice(10, 14)
                )}
              </div>
            </div>

            <div id="mobileNavCont" className="bg-black w-full sm:hidden">
              <div className="w-mainW flex justify-between mx-auto text-gray-300 ">
                <h1 className="capitalize  text-2xl py-5 font-bold text-white ">
                  feliciano
                </h1>
                <div className="flex gap-2 justify-between items-center">
                  <svg
                    width="27px"
                    viewBox="0 0 24 24"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      d="M4 18L20 18"
                      stroke="#d1d5dc"
                      stroke-width="2"
                      stroke-linecap="round"
                    />
                    <path
                      d="M4 12L20 12"
                      stroke="#d1d5dc"
                      stroke-width="2"
                      stroke-linecap="round"
                    />
                    <path
                      d="M4 6L20 6"
                      stroke="#d1d5dc"
                      stroke-width="2"
                      stroke-linecap="round"
                    />
                  </svg>
                  <h1 className="text-xl">Menu</h1>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </>
  );
}

export default Header;
